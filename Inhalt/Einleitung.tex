...
im Vergleich zu anderen Industriezweigen mit einer signifikanten Digitalisierungslücke konfrontiert \citep{BBSR}.
...
Dies führt regelmäßig zum sogenannten „Excel-Fallback“ \citep{feth2023studie}: 
...
\newline
Zusätzlich verschärft das Problem des Vendor-Locks die Situation \citep{feth2023studie}.
...
die den spezifischen Anforderungen komplexer Bauprozesse, wie einem durchgängigen Inbetriebnahmemanagement, nicht gerecht werden
\newline
Erschwert wird der digitale Einsatz vor Ort durch die oft mangelhafte Infrastruktur \citep{BBSR}. 
...
Um dies zu ermöglichen, bedarf es eine standardisierte und transparente Schnittstellen um einen problemlosen Informationsaustausch zu gewährleisten. 
Es existieren bereits Forschungen und etablierte Ansätze für graphbasierte Domänenmodelle, sowie die Nutzung von GraphQL als Vermittlungsschicht, um die komplexen Strukturen des IFC Standards abzubilden. Arbeiten wie \citep{wolf:2025:ifc2graphql} zeigen bereits,
dass die Überführung von IFC-Daten in ein GraphQL Schemata grundlegend machbar ist, jedoch eine exakte 1:1 Abbildungen an ihre Grenzen hinsichtlich der Komplexität stößt. Trotzdem wird deutlich, dass eine solche Überführung Vorteile in Form von Effizient
und Entwicklerfreundlichkeit mit sich bringt. \newline
Es fehlt jedoch an einer Verknüpfung dieser Modelle mit einer modularen Architektur als Frontend, mit welcher die beschriebenen Probleme aus \label{1.1}, gezielt angegangen werden können. 
...
\subsection{Methodik}
Um die beschriebenen Zielsetzung zu erreichen wird diese Arbeit in drei Teilschritte unterteilt. Zuerst soll durch Literaturrecherche und prototypischen Tests ein Domänenmodell mit der Graphdatenbank \textit{Neo4j} und dem Framework \textit{GraphQL} als Vermittlerinstanz
auf seine Vorteile und Machbarkeit untersucht werden. Als nächster Schritt soll eine modulare Softwarearchitektur, aufbauend auf dem Konzept der Microfrontends, entworfen werden. Die technische Umsetzung soll mit dem Build-Tool \textit{Vite} und dem Architekturmuster
\textit{Module Federation} erfolgen. Das finale Ziel dieses Schrittes ist ein Prototyp als Proof of Concept. Das Zusammenspiel dieser beiden Technologien bietet eine moderne und entwicklerfreundliche Möglichkeit, Mircofrontends als Webapplikationen herauszuführen. \newline
